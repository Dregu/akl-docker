...
ENV TERM xterm
CMD bash
RUN dpkg --add-architecture i386 && apt-get update && apt-get install -y lib32gcc1 libstdc++6:i386 curl tmux bsdmainutils wget mailutils postfix git nano unzip binutils \
...
./srcds_run -game csgo -usercon -strictportbind -ip $(hostname --ip-address) -port 27015 +clientport 27005 +tv_port 27020 -tickrate $TICKRATE +sv_setsteamaccount $GSLT +map $MAP +servercfgfile \
csgo-server.cfg -maxplayers_override $MAXPLAYERS +mapgroup $MAPGROUP +game_mode $GAMEMODE +game_type $GAMETYPE +net_public_adr $PUBLIC $CUSTOM' >> /home/cs/start) \
